...
let
  ...
  userChrome_preferences =
    if ENABLE_TOP_SEARCHBAR_USERCHROME then (import ./userChrome_preferences.nix { }) else { };

  top_searchbar_userChrome = import ./top_searchbar_userChrome.nix;
  top_searchbar_userChrome_attrs =
    profile_name:
    if ENABLE_TOP_SEARCHBAR_USERCHROME then
      {
  ...
  mkProfile =
    name: id: use_containers: preferences: enable_top_searchbar_userChrome: extensions: additional_attrs: {
      "${name}" = {
        inherit name id;
        ...
        settings =
          ...
          // preferences
          // (if enable_top_searchbar_userChrome then userChrome_preferences else { });
      ...
      // (if enable_top_searchbar_userChrome then top_searchbar_userChrome_attrs name else { })
      // additional_attrs;
    };

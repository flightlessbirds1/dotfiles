...
let
  inherit (flake.self.packages.${pkgs.stdenv.hostPlatform.system})
    haskeww
    ;
  inherit (builtins)
    filter
  ...
  processYuckFile =
    file: builtins.replaceStrings [ "pathto" ] [ configPath ] (builtins.readFile file);
  ...
  xdgConfig = listToAttrs (
    map (
      name:
      if lib.hasSuffix ".yuck" (toString name) then
        {
          name = builtins.replaceStrings [ ("eww/" + configSource) ] [ "eww" ] (
            builtins.toString (builtins.tail (lib.splitString "wayland/" (toString name)))
          );
